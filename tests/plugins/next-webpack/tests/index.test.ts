...
const buildNextWebpack = async (): Promise<void> => {
    await new Promise<void>((resolve, reject): void => {
        ...
        child.on("close", (code: number | null): void => {
            ...
            reject(
                new Error(
                    [
                        `next build --webpack exited with code ${code ?? "null"}`,
                        stdout,
                        stderr,
                    ].join("\n"),
                ),
...
describe("next webpack test", (): void => {
    it("should build via Next.js webpack", async (): Promise<void> => {
        await buildNextWebpack();
    });
